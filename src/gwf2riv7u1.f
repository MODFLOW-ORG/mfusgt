      MODULE GWFRIVMODULE
        ...
        INTEGER,SAVE,POINTER  ::NPRIV,IRIVPB,NNPRIV,MERGE_KRIV,IBINRIV
...
C     ------------------------------------------------------------------
     ...
     1                  IRIVPB,NNPRIV,RIVAUX,RIVR,MERGE_KRIV,IBINRIV
C
      CHARACTER*400 LINE
C     ------------------------------------------------------------------
C
...
C4------READ AUXILIARY VARIABLES AND PRINT OPTION.
   ...
   13    FORMAT(1X,'LISTS OF RIVER CELLS WILL NOT BE PRINTED')
         IPRRIV = 0
         GO TO 10 
      ELSE IF(LINE(ISTART:ISTOP).EQ.'MERGE_BED_K1') THEN
         WRITE(IOUT,14)
   14    FORMAT(1X,'RIVER CONDUCTANCE WILL BE COMPUTED AS HARMONIC',
     1    1X,'MEAN OF RIVER BED COND AND VERTICAL K OF ASSOCIATED CELL')
         MERGE_KRIV = 1
         GO TO 10
      ELSE IF(LINE(ISTART:ISTOP).EQ.'BINARY') THEN
C IHM-----READ KEYWORD OPTION TO USE BINARY INPUT FILE FORMATS
         CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IB,R,IOUT,IN)
         IBINRIV = IB
         WRITE(IOUT,15) IBINRIV
15       FORMAT(1X,'RIVER DATA INPUT ARE IN BINARY FORMATS FROM FILE'
     1     1X,'ON UNIT',I5)
         GO TO 10         
      END IF
C
...
C     ------------------------------------------------------------------
     ...
     1                  IRIVPB,NNPRIV,RIVAUX,RIVR,MERGE_KRIV,IBINRIV
csp! IHM - Added to allow binary input in RIV RP package
      INTEGER AN,AK,AI,AJ
      character*200 line
C     ------------------------------------------------------------------
C
...
C1------NUMBER OF PARAMETERS.
      IF(IBINRIV.EQ.0) THEN   ! READ FORMATTED FILES
        CALL URDCOM(IN,IOUT,LINE)
        IF(NPRIV.GT.0) THEN
           IF(IFREFM.EQ.0) THEN
              READ(line,'(2I10)') ITMP,NP
           ELSE
              READ(line,*) ITMP,NP
           END IF
        ELSE
...
C3------IF THERE ARE NEW NON-PARAMETER REACHES, READ THEM.
      MXACTR=IRIVPB-1
      IF(ITMP.GT.0) THEN
        IF(IBINRIV.EQ.0) THEN   ! IHM READ FORMATTED FILES          
   ...
   99       FORMAT(1X,/1X,'THE NUMBER OF ACTIVE REACHES (',I6,
     ...
     3          RIVAUX,20,NAUX,IFREFM,NEQS,5,5,IPRRIV)
         ENDIF
        ELSE
C3A -----READ BINARY RIV FILE
          IF(IPRRIV.NE.0) THEN
! ---------- SET UP HEADER FOR PRINTING
            IF(IUNSTR.EQ.0) THEN
              WRITE(IOUT,66)
            ELSE
              WRITE(IOUT,67)
            ENDIF
          ENDIF
C3B ------READ ALL ACTIVE RIV CELLS FOR THIS STRESS PERIOD
          DO II = 1,ITMP
            IF(IUNSTR.EQ.0) THEN
              READ(INB)AK,AI,AJ,(RIVR(JJ,II),JJ=4,NRIVVL-1)
              RIVR(1,II) = AK
              RIVR(2,II) = AI
              RIVR(3,II) = AJ
              IF(IPRRIV.NE.0) THEN
                ...
                I = AI
                J = AJ
                WRITE(IOUT,68)K,I,J,(RIVR(JJ,II),JJ=4,NRIVVL-1)
              ENDIF
            ELSE
              READ(INB)AN,(RIVR(JJ,II),JJ=4,NRIVVL-1)
              RIVR(1,II) = AN
              IF(IPRRIV.NE.0) THEN
                N = AN
                WRITE(IOUT,69)N,(RIVR(JJ,II),JJ=4,NRIVVL-1)
              ENDIF
            ENDIF
          ENDDO
        ENDIF
      END IF
66    FORMAT(/10X,'LIST OF RIV NODES'/10X,25('-')/ 10X,'LAYER',5X,'ROW',
     * 2X,'COLUMN',7X,'STAGE',4X,'CONDFACT',8X,'RBOT')
67    FORMAT(/10X,'LIST OF RIV NODES'/10X,25('-')/ 10X,'NODE',7X,
     * 'STAGE',4X,'CONDFACT',8X,'RBOT')
68    FORMAT(7X,3I8,3G12.5)
69    FORMAT(8X,I8,100G12.5)
C      
      NRIVER=NNPRIV
C
...
C-------FOR STRUCTURED GRID, CALCULATE NODE NUMBER AND PLACE IN LAYER LOCATION
      IF(ITMP.GT.0.AND.IUNSTR.EQ.0)THEN
        DO L=1,NNPRIV  ! ONLY NEEDS TO BE DONE FOR NON-PARAMETER RIVERS
          IR=RIVR(2,L)
          IC=RIVR(3,L)
          IL=RIVR(1,L)
      ...
      ENDIF
C ---------------------------------------------------------------------
C        PREPARE RIVER CONDUCTANCE TERM IF MERGING WITH CELL VERTICAL K
C ---------------------------------------------------------------------         
         IF(ITMP.GT.0.AND.MERGE_KRIV.EQ.1) THEN
           CALL MERGE_CONDUCTANCE(RIVR, NNPRIV, MXRIVR,NRIVVL)  
         ENDIF      
C
C8------RETURN.
  260 RETURN
      END
C ----------------------------------------------------------------------
      SUBROUTINE MERGE_CONDUCTANCE(RIVR, NNPRIV, MXRIVR,NRIVVL)
C     ******************************************************************
C     COMPUTE HARMONIC MEAN OF BED CONDUCTANCE AND CELL CONDUCTANCE 
C     ******************************************************************
C
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: AREA,NEQS,NODES,IOUT,NODLAY,BOT,TOP,STRT,IUNIT
      USE GWFBCFMODULE, ONLY: CV,VKA,LAYVKA,IKVFLAG,LAYSTRT,HK 
      DIMENSION RIVR(NRIVVL,MXRIVR)
      DOUBLE PRECISION CRIV, COND, HYC1, VK1,BBOT,TTOP,ELCELL,ELBOT,DIST
C
C1 --------CHECK IF VK IS AVAILABLE FOR BCF PACKAGE
      IF(IUNIT(1).GT.0.AND.IKVFLAG.EQ.0)THEN
        WRITE(IOUT,20) 
        RETURN 
      ENDIF 
20    FORMAT(/1X,'*** KV NOT AVAILABLE IN BCF PACKAGE, COMBINED' 
     1   1X, 'BED-LEAKANCE IS NOT COMPUTED ***')           
C2 --------LOOP OVER ALL RIVER CELLS AND PERFORM ADJUSTMENT
      DO L=1,NNPRIV  
        N=RIVR(1,L)
C3 --------FOR LPF PACKAGE
        IF(IUNIT(23).GT.0)THEN
C4 --------FIND LAYER OF CELL TO DETERMINE VK FROM VANI            
          DO K = 1,NLAY
            NNDLAY = NODLAY(K)
            NSTRT = NODLAY(K-1)+1            
          ...
          ENDDO  
10        CONTINUE   
C5 --------CALCULATE VERTICAL HYDRAULIC CONDUCTIVITY FOR CELL IN HYC1.
          HYC1 = 0.0
...
C6 --------FOR BCF PACKAGE
        IF(IUNIT(1).GT.0)THEN 
          ...
          ENDIF  
        ENDIF
C7 --------CALCULATE COMBINED CONDUCTANCE FOR RIVER AND CELL AS HARMONIC MEAN.
        BBOT=BOT(N)
        TTOP=TOP(N)
        IF(IUNIT(23).NE.0)THEN
          IF(LAYSTRT(ILAY).NE.0) TTOP=STRT(N)
        ENDIF  
        ELCELL = 0.5 * (TTOP + BBOT)
        ELBOT = RIVR(6,L)
        DIST = ELBOT - ELCELL
        IF(DIST.GT.0.0)THEN
          VK1=AREA(N) * HYC1 / DIST
          CRIV = RIVR(5,L)
          COND = 1.0 / VK1 + 1.0 / CRIV
          RIVR(5,L) = 2.0 / COND
        ENDIF  
      ENDDO  
C
