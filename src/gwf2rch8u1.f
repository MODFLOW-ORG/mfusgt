...
C
        ...
        DEALLOCATE(IPONDOPT,ICONCRCHOPT, STAT = ALLOC_ERR)
...
C-------RTS-related allocations
        IF(IRTSOPT.EQ.1)THEN
          DEALLOCATE(MXZNRCH, STAT = ALLOC_ERR)
          DEALLOCATE(TSTARTRCH,TENDRCH, STAT = ALLOC_ERR)
          DEALLOCATE(FACTRRCH,RTSRCH, STAT = ALLOC_ERR)
        ENDIF
        DEALLOCATE(IRTSOPT)
