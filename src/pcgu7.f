...
C     + + + DUMMY ARGUMENTS + + +
      ...
      DOUBLEPRECISION, INTENT(IN)                             :: Rcutoff
...
C-------SOLUTION BY THE CONJUGATE GRADIENT METHOD      
      IF ( ILINMETH.EQ.1 ) THEN
        CALL SPCGU_CG(ICNVG,ICUTOFF,itmax,innerit,Rcutoff)
C-------SOLUTION BY THE BICONJUGATE GRADIENT STABILIZED METHOD      
      ELSE IF ( ILINMETH.EQ.2 ) THEN
        CALL SPCGU_BCGS(ICNVG,ICUTOFF,itmax,innerit,Rcutoff)
      END IF
C
...
C---------RETURN
      ...
      SUBROUTINE SPCGU_CG(ICNVG,ICUTOFF,ITMAX,INNERIT,Rcutoff)
        !USE PCGUMODULE
        IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
        INTEGER, INTENT(INOUT) :: ICNVG
...
C-----------UPDATE X AND RESIDUAL
          ...
          DO n = 1, NIAC
            ...
            DC(n)  = t
            rmax   = MAX( ABS(t), rmax )
            ! DM: Compute sum of squares residual in addition to max residual
            rms  = rms + (t ** 2)
            ! End DM
          END DO
          CALL SPCGU_TESTCNVG( IITER,ICNVG,ICUTOFF,
     2                         deltax,rmax,dhclose,drclose,Rcutoff,rms )
C           CHECK FOR EXACT SOLUTION
...
C---------RETURN
      ...
      SUBROUTINE SPCGU_BCGS(ICNVG,ICUTOFF,ITMAX,INNERIT,Rcutoff)
        !USE PCGUMODULE
        IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
        INTEGER, INTENT(INOUT) :: ICNVG
...
C---------TEST FOR SOLVER CONVERGENCE
        SUBROUTINE SPCGU_TESTCNVG( IITER,ICNVG,ICUTOFF,
     2                             Hmax,Rmax,Hclose,Rclose,Rcutoff,Rms )
...
C     + + + CODE + + +
        IF ( ABS(Hmax).LE.Hclose .AND. ABS(Rmax).LE.Rclose ) THEN
          ICNVG = 1
        END IF
        ! DM: Always cut off when sum of squares residual hits Rcutoff if it has been supplied (non-negative Rcutoff)
        IF(ICNVG.EQ.0.AND.Rcutoff.GE.0.0D0.AND.ABS(Rms).LE.Rcutoff) THEN
          ICNVG = 1
          ICUTOFF = 1
          !WRITE(IOUT,*)'R sum of squares of ',Rms,' <= Rcutoff, stopping.'
        END IF
C---------RETURN
        RETURN
...
C Complex
      ...
      END SUBROUTINE
C -----------------------------------------------------------------------
      SUBROUTINE PCGUSETDIMS(NEQS2,NJA2)
      USE GLOBAL,   ONLY:IOUT, IAC=>IA2, JAC=>JA2
          NIAC=NEQS2
          NNZC=NJA2
C       GENERATE IAPC AND JAPC
      IF ( IPC.EQ.2 .OR. IPC.EQ.3 ) THEN
