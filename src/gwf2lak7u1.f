...
C       argument lists are defined
      ALLOCATE (NLAKES, NLAKESAR,THETA,LAKUNIT,ILKTRNSPT,IETLAK)
      NLAKES = 0
      LAKUNIT = IN
      NLAKESAR = 1
      THETA = 0.0
      ILKTRNSPT=0
      IF(IUNITGWT.NE.0) ILKTRNSPT=1
      IETLAK=0 !TEMPORARY HARDWIRING - MASS DOES NOT LEAVE WITH ET
      FOUND=.FALSE.
      IF (IN.GT.0) THEN
...
! Check for alternate option to specifiy stage/vol/area tables.
      ...
      lloc = 1
110   CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,I,R,IOUT,IN)
   ...
   32    FORMAT(1X,' Stage, volume and area relationship specified ',
     +                'based on an external tabular input file')
      ...
      ELSEIF(LINE(ISTART:ISTOP).EQ.'TRANSPORTBOUNDARY') THEN
        ILKTRNSPT=0
        FOUND=.TRUE.
      END IF
      IF(LLOC.LT.200) GO TO 110
C.....WRITE MESSAGE IF TABULAR INPUT IS NOT USED      
      IF(IRDTAB.EQ.0) THEN  
        WRITE(IOUT,'(A)') ' Model grid will be used to develop ',
     +                     ' volume and area relationship. '
      END IF      
C
C.....WRITE LAKE TRANSPORT MESSAGES
      IF(IUNITGWT.EQ.0) THEN
        IF(FOUND) THEN
          WRITE(IOUT,'(10A)') ' Solute transport is not simulated'
          WRITE(IOUT,'(10A)') ' TRANSPORTBOUNDARY option is invalid ',
     1                        'and will be ignored'
        ENDIF
      ELSE
        IF(ILKTRNSPT.EQ.0) THEN
          WRITE(IOUT,'(10A)') ' TRANSPORTBOUNDARY option is active. ',
     1                      'Lakes will act like a boundary ',
     1                      'condition for groundwater. Transport ',
     1                      'within lakes will not be simulated.'
        ELSE
          WRITE(IOUT,'(10A)') 
     1      ' Transport within lakes will be simulated.'
        ENDIF
      ENDIF
C
...
C
C Lakes are active
      ALLOCATE (STAGES(NLAKESAR))
      STAGES = 0.0
      IF(IUNITGWT.GT.0) THEN
        ALLOCATE (CLAKE(NLAKESAR,NSOL))
        CLAKE = 0.0
      ENDIF
      IF(ILKTRNSPT.EQ.1) THEN
        ALLOCATE (CLKOLD(NLAKESAR,NSOL),CLAKINIT(NLAKESAR,NSOL))
        ALLOCATE (CAUG(NLAKES,NSOL),CPPT(NLAKES,NSOL),CRNF(NLAKES,NSOL))
        ALLOCATE (RMASLAK(NLAKES),VOUTLAK(NLAKES))
        ALLOCATE (CGW2LAK(NSOL),CUZF2LAK(NSOL),CGWFROMLAK(NSOL),
     1       CSFR2LAK(NSOL),CSFRFROMLAK(NSOL),CPRECLK(NSOL),
     1       CRUNOFLK(NSOL),CWDRLLK(NSOL),CETLK(NSOL),
     1       CSTORINLK(NSOL),CSTOROTLK(NSOL))
        CLKOLD = 0.0
        CLAKINIT = 0.0
        CAUG = 0.0
        CPPT = 0.0
...
Cdep     1'CONVERGENCE CRITERION = ',1PE9.2)
        ...
        ALLOCATE (RNF(NLAKES), CUMRNF(NLAKES))
...
CVSB            WRITE(iout,*)'Transport not correct for USGs',
CVSB     1                    '--program stopping'
CVSB            WRITE(iout,*)'Please change input and restart program'
CVSB            CALL USTOP(' ')
            DO 35 LM=1,NLAKES
               IF (IFREFM.EQ.0) THEN
 ...
 35           WRITE (IOUT,LFRMAT) LM,(CLAKE(LM,ISOL),ISOL=1,NSOL)
cgage
            CLAKINIT=CLAKE
            CLKOLD=CLAKE
         END IF
...
9     FORMAT(1X,I3,4X,1P,3E10.3,1X,5E10.3)
     ...
     1 RNF(LM),WTHDRW(LM),BOTTMS(LM),BGAREA(LM)
      ...
      IF (ILKTRNSPT.EQ.0) THEN !LAKE AS A BOUNDARY
        IF(IFREFM.EQ.0) THEN
          READ(IN,'(100F10.4)') (CLAKE(LM,ISOL),ISOL=1,NSOL)
        ELSE
          READ(IN,*) (CLAKE(LM,ISOL),ISOL=1,NSOL)
        END IF
      ELSE !LAKE TRANSPORT
      DO 850 ISOL=1,NSOL
        IF(IFREFM.EQ.0) THEN
  ...
  842   FORMAT(1X,I3,6X,1P,100E10.2)
  850 CONTINUE
      ENDIF
C      WRITE (IOUTS,'(/)')
  300 CONTINUE
C.....WRITE TRANSPORT BOUNDARY
      IF (IUNITGWT.GT.0) THEN
        IF (ILKTRNSPT.EQ.0) THEN !LAKE AS A BOUNDARY
          WRITE (IOUT,10)
          DO LM=1,NLAKES
            WRITE(IOUT,842) LM,(CLAKE(LM,ISOL),ISOL=1,NSOL)
          ENDDO
        ELSE !LAKE TRANSPORT
          WRITE (IOUT,8)
          DO LM=1,NLAKES
          DO ISOL=1,NSOL
            IF(WTHDRW(LM).LT.0.0)
     1      WRITE(IOUT,840) LM,ISOL,CPPT(LM,ISOL),CRNF(LM,ISOL),
     1      CAUG(LM,ISOL)
            IF(WTHDRW(LM).GE.0.0)
     1      WRITE(IOUT,841) LM,ISOL,CPPT(LM,ISOL),CRNF(LM,ISOL)
          ENDDO
          ENDDO
        ENDIF
      ENDIF
 8    FORMAT (/1X,'LAKE',4X,'SOLUTE',6X,'CPPT',6X,'CRNF',6X,'CAUG'
     1        /55('-'))
10    FORMAT (/1X,'LAKE',6X,'CLAKE(1:NSOL)'/25('-'))
C
      WRITE (IOUT,'(/)')
C
...
C
         ...
         DO II = I1,I2
           JJ = JA(II)
           IIS = JAS(II)
           IF(IVC(IIS).EQ.0)THEN  ! HORIZONTAL CONNECTION
...
C
  ...
  680 FORMAT(/1X,'NUMBER OF CONNECTED LAKE SYSTEMS IN SIMULATION IS ',I3
     1)
      ...
      WRITE(IOUT,18) IS, ICS(IS), ISUB(IS,1)
   18 FORMAT(/10X,'SYSTEM',I3//2X,'NUMBER OF LAKES IN SYSTEM',I5,
     1  '  CENTER LAKE NUMBER',I8//1X,'SUBLAKE NUMBER',3X,
     2  'SILL ELEVATION'/)
      DO 715 JK=2,IC
  715 WRITE(IOUT,717) ISUB(IS,JK), SILLVT(IS,JK-1)
  717 FORMAT(8X,I2,8X,F10.2)
...
C     ------------------------------------------------------------------
     ...
     +                        BOTTMS, IDIV, STGOLD2, NDV,ILKTRNSPT,FLOB,
     +                        CLKOLD,CLAKE
...
Cdep  initialized VOLINIT and VOLOLD to VOLOLDD 6/4/2009
      DO I=1,NLAKES
        IF(KKPER.EQ.1.AND.KKSTP.EQ.1) THEN
          ...
          STGNEW(I)=STAGES(I)
        ELSE
          IF(STGNEW(I).GT. 999998.0) STGNEW(I) = STAGES(I)   !CSP NEED THIS TO TURN ON DIFFERENT LAKES IN DIFFERENT SPS
          STGOLD2(I)=STGNEW(I)
          STGOLD(I)=STGNEW(I)
          VOLOLDD(I)=VOLTERP(STGOLD(I),I)
          VOLOLD(I)=VOLOLDD(I)
...
C                            aquifer)
      IF (IUNITGWT.GT.0) THEN
        DO 50 LK=1,LKNODE
 50        FLOB(LK)=0.0
        IF(ILKTRNSPT.EQ.1) CLKOLD=CLAKE
      END IF
C
C3------RETURN
      RETURN
...
C17D----NEW LAKE STAGE COMPUTED FROM LAKE VOLUME.
              ...
              IF(STGNEW(LAKE).LT.BOTTMS(LAKE))
     +             STGNEW(LAKE)=BOTTMS(LAKE)
C17E-----IF LAKE IS INACTIVE THEN KEEP STAGE FROM STGOLD ----------------------
              IF(BOTTMS(LAKE). GT. 999998) STGNEW(LAKE) = STGOLD(LAKE)
CSP-----------------------------------------------------------------------------              
            END IF
        END DO
...
C-LFK         IF(IAUXSV.NE.0) NAUX=1
        ...
        ENDDO                                                           
        IF(IGWTOUT.EQ.0)
     1   CALL UBDSVHDR(IUNSTR,KSTP,KPER,IOUT,ILKCB,IICLNCB,NODES,
...
! Save seepage to UZF for writing Lake-to-UZF in GAG Package.
                 ...
                 RATE=FLOBOT
                 IF (IUNITGWT.GT.0) FLOB(L)=FLOB(L)+FLOBOT
          IF(IGWTOUT.EQ.0) THEN
...
899              IF(IBD.EQ.2) THEN    !RGN need to fix this for USGs
                   ...
                   R=RATE
                   IF(IGWTOUT.EQ.0)
     1             CALL UBDSVREC(IUNSTR,NL,NODES,NNCLNNDS,ILKCB,IICLNCB,
     1    1,1,NAUX,R,FACE(1),IBOUND,NCOL,NROW,NLAY)
                 END IF
               END IF
           END DO
         END DO
...
!dep replaced stgold2 and stages with delh and tdelh
      IF (IUNITGAGE.GT.0)THEN
        IF(IUNITGWT.EQ.0.AND.IGWTOUT.EQ.0) THEN
          CALL SGWF2GAG7LO(IUNITGWT,IUNITUZF,TOTIMREAL,
     +         GWIN,GWOUT,SEEP,FLXINL,VOLOLD,NSOL)
        ELSEIF(IUNITGWT.GT.0.AND.IGWTOUT.EQ.1) THEN
          CALL SGWF2GAG7LO(IUNITGWT,IUNITUZF,TOTIMREAL,
     +         GWIN,GWOUT,SEEP,FLXINL,VOLOLD,NSOL)
        END IF
      END IF
...
Cdep  deallocate SURFDEPTH 3/3/2009
      ...
      DEALLOCATE (SUMCNN)
      DEALLOCATE (SUMCHN)
      IF(ASSOCIATED(CLAKE)) DEALLOCATE (CLAKE)
      IF(ASSOCIATED(CRNF)) DEALLOCATE (CRNF)
      IF(ASSOCIATED(CAUG)) DEALLOCATE (CAUG)
      IF(ASSOCIATED(CPPT)) DEALLOCATE (CPPT)
      IF(ASSOCIATED(CLAKINIT)) DEALLOCATE (CLAKINIT)
      IF(ASSOCIATED(CLKOLD)) DEALLOCATE (CLKOLD)
      DEALLOCATE (SILLVT)
      DEALLOCATE (BDLKN1)
