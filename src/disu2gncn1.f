...
c                CONSTERM = - PGF(IIS)*ALPHA *(HNEW(N1) - HNEW(Jn)) !should not have negative term
                IF(IUPS.EQ.N1)THEN
...
C10---------COMPUTE GHOST K-VALUE AND HORIZONTAL CONDUCTANCE AS PER LAYCON
          ...
          IF(LAYCON(K).EQ.3. OR.LAYCON(K).EQ.1) THEN
            IF(ND1.GT.NODES.OR.ND2.GT.NODES) CYCLE ! CORRECTION ON CLN ONLY FOR LAYCON=4 OR 5
...
C     ******************************************************************
C     FILL SECOND ORDER CORRECTION OF K ON UNCONFINED GHOST NODE TERMS
C     AND COMPUTE HORIZONTAL CONDUCTANCE AND ADJUST AMAT FOR LAYCON=4 OR 5
C     ******************************************************************
...
C     ******************************************************************
      USE GLOBAL, ONLY:HNEW,TOP,BOT,Sn,NODLAY,NLAY
      USE GNCnMODULE,ONLY:NGNCn,GNCn,I2Kn,MXADJn
      DOUBLE PRECISION HD,BBOT,TTOP,THCK,TOTTHICK,SUPS,ALPHA,ALPHAN   
