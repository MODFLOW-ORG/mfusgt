...
c                  <  0 nothing printed
c                  >= 0 print all
c     mrcutoff    sum of squares residual cutoff for trunacted Newton
c     micutoff    cutoff flag for trunacted Newton (=1 if converged due to mrcutoff)
c
      integer ::  miunit, miout

      ! DM: Truncated newton cutoff residual and flag
      double precision :: mrcutoff
      integer :: micutoff
      ! End DM
...
!       alpha = temp2/temp1
     ...
     [                   dabs(omegah*qb(i) + alpha*sb(i)), temp1 )
        ...
        ! DM: Stop solution at sum of squares residual cutoff if specified (mrcutoff >= 0)
        micutoff = 0
        conv = (mrcutoff >= 0.0D0).and.(resmax <= mrcutoff)
        if(conv) micutoff = 1
        resmax = dsqrt(resmax)
        conv = conv.or.(temp1 < ctol)
...
c     initialize q and aq work arrays (cd1 2/22/2019)
      do i=1,nblack
          q(i) = 0.0
          aq(i) = 0.0
      enddo    
...
c
     ...
     [                   rcutoff, icutoff)
      ...
      ! DM: Additional arguments for truncated Newton (residual cutoff and flag)
...
c
      ...
      ! DM: Store sum of squares residual cutoff and initialise cutoff flag
      mrcutoff = rcutoff
      micutoff = 0
      ! End DM
...
c
     ...
     [                nred, n, nja, njaf, nitmax, north, ierr)
      ...
      ! DM: Return truncated Newton cutoff flag
      icutoff = micutoff
      ! End DM
...
C      IF(.NOT.ALLOCATED(iaf)) ALLOCATE(iaf(nblack+1), stat = ierror)
C      IF(.NOT.ALLOCATED(idiagf)) ALLOCATE(idiagf(nblack), stat = ierror)
