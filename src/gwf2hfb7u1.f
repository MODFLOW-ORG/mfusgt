      MODULE GWFHFBMODULE
        INTEGER, POINTER :: MXHFB,NHFB,IPRHFB,NHFBNP,NPHFB,IHFBPB,ITRHFB
...
C1------Allocate scalar data.
      ALLOCATE(MXHFB,NHFB,IPRHFB,NHFBNP,NPHFB,IHFBPB,ITRHFB)
C
C2------IDENTIFY PACKAGE.
      WRITE(IOUT,1) INHFB
...
10    CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,INHFB)
    ...
    3   FORMAT(1X,
     &'LISTS OF HORIZONTAL FLOW BARRIER CELLS WILL NOT BE PRINTED')
        IPRHFB = 0
      ELSEIF(LINE(ISTART:ISTOP).EQ.'TRANSIENT_HFB') THEN
        ITRHFB = 1
        WRITE(IOUT,4)
    4   FORMAT(1X,
     &'TRANSIENT HORIZONTAL FLOW BARRIER CELLS OPTION SELECTED')  
...
C
      ...
      END      
C --------------------------------------------------------------------------      
      SUBROUTINE GWF2HFB7U1RP(INHFB,KPER)
C     ******************************************************************
C     READ AND PREPARE FOR HORIZONTAL FLOW BARRIER PACKAGE TRANSIENT PROPERTY
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C
      IF(ITRHFB.GT.0)THEN
C2 ----READ INDEX TO READ HFBS IF TRANSIENT HFBS      
          READ(INHFB,*) IHFBRD
      ELSE 
C3 -----IF STEADY HFB THEN READ HFB INFO ONLY FOR FIRST STRESS PERIOD          
        IF(KPER.EQ.1) THEN
          IHFBRD = 1  
        ELSE
          IHFBRD = 0
      ...
      ENDIF
C
C4 ----PRINT STATEMENT IN LISTING FILE AND RETURN IF HFB INFO IS NOT READ
      IF(IHFBRD.LE.0) THEN 
        IF(ITRHFB. GT. 0) WRITE(IOUT,4) ITRHFB
4       FORMAT(/10X,
     1   '*** HFB INFO NOT READ FOR THIS STRESS PERIOD, ITRHFB =',I5,
     1    ' USING INFORMATION FROM PREVIOUS STRESS PERIOD ***')
        RETURN
      ENDIF  
C
C5------READ PARAMETER DEFINITIONS (ITEMS 2 AND 3)
  ...
  600 FORMAT(//,1X,I5,' HFB parameters')
   ...
   20   CONTINUE
      ENDIF
C
C6------READ BARRIERS NOT DEFINED BY PARAMETERS (ITEM 4)
      NHFB = 0
  ...
  610 FORMAT(/,1X,I6,' BARRIERS NOT DEFINED BY PARAMETERS')
     ...
     2             NODES,NODLAY,NLAY,IPRHFB)
      ...
      ENDIF
C
C7------SUBSTITUTE DATA FOR PARAMETERIZED BARRIERS INTO ACTIVE SECTION
C7------OF HFB ARRAY
      IOUTU = IOUT
      ...
      IF(NPHFB.GT.0) THEN
C
C8------READ NUMBER OF ACTIVE HFB PARAMETERS (ITEM 5)
        READ(INHFB,*) NACTHFB
        IF (NACTHFB.GT.0) THEN
          DO 650 I = 1,NACTHFB
C
C9------READ AND ACTIVATE AN HFB PARAMETER (ITEM 6)
...
C      
C
C10 -----MODIFY HORIZONTAL BRANCH CONDUCTANCES FOR CONSTANT T LAYERS.
...
C ------FIND BARRIER LOCATION AND ADJUST PGF
        DO IAJ = IA(N1)+1,IA(N1+1)-1
          ...
          IF(JJ.EQ.N2)THEN
            ...
            ENDIF  
            TDW = THKAVG*HFB(6,II)              
            IF(KPER.EQ.1) HFB(7,II) = PGF(IAJS)
            PGF(IAJS) = TDW*HFB(7,II)*FAHL(IAJS)/    
     *       (TDW*FAHL(IAJS) + HFB(7,II))
            GO TO 100
          ENDIF
        ENDDO
100     CONTINUE
