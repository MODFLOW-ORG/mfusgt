...
C  Deallocate ETS MEMORY
      ...
      INTEGER ALLOC_ERR      
C
        DEALLOCATE(NETSOP, STAT = ALLOC_ERR)
        DEALLOCATE(IETSCB, STAT = ALLOC_ERR)
        DEALLOCATE(NPETS, STAT = ALLOC_ERR)
        DEALLOCATE(IETSPF, STAT = ALLOC_ERR)
        DEALLOCATE(NETSEG, STAT = ALLOC_ERR)
        DEALLOCATE(MXNDETS, STAT = ALLOC_ERR)
        DEALLOCATE(INIETS,NIETS, STAT = ALLOC_ERR)
        DEALLOCATE(IETS, STAT = ALLOC_ERR)
        DEALLOCATE(ETSR, STAT = ALLOC_ERR)
        DEALLOCATE(ETSX, STAT = ALLOC_ERR)
        DEALLOCATE(ETSS, STAT = ALLOC_ERR)
        DEALLOCATE(PXDP, STAT = ALLOC_ERR)
        DEALLOCATE(PETM, STAT = ALLOC_ERR)
C-------Transport-related allocations
        IF(INBCT.GT.0)THEN
          DEALLOCATE(ETSF, STAT = ALLOC_ERR)
          DEALLOCATE(ESFACTOR, STAT = ALLOC_ERR)
        ENDIF
C
      RETURN
      END
